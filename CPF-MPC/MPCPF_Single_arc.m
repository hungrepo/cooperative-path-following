...
%gamma0=0;
gamma0=0;
a=10;omega=1;phi=-pi;d=20;
path_data=[a;omega;phi;d];
x_path0=ini_path(path_data,gamma0);
input_path.Ts = Ts;
input_path.nSteps = 4;
...
import casadi.*
[nlp_sol,w0,lbw,ubw,lbg,ubg,nx,nu,k1,k2,k3]=NLP_gamma(Np,Tp,Ts,path_data);
...
%     cs=1/a;
%     if x0(3)==0
%         r_d=-k3*x0(2)*vd/(1+x0(1)^2+x0(2)^2)+cs*v_old;
%     else
%         r_d=-k3*x0(2)*vd*sin(x0(3))/(x0(3)*(1+x0(1)^2+x0(2)^2))-k2*tanh(x0(3))+cs*v_old;
%     end
%     v=vd*cos(x0(3))+k1*tanh(x0(1));
%     v_old=v;  
%%    
 ...
 %% Update vehicle and path   
  ...
  % integrate vehicle
    u_d=vd;
    u_robot=[u_d;r_d];
    U_robot=[U_robot u_robot];
    x_robot(:,end+1)=update_vehicle(x_robot(:,end),u_robot,Ts);
    % integrate path
    input_path.u = v;
    U_path=[U_path v];
    input_path.x = x_path(3:4,end);
    output_path = RK4_integrator(@(t,x,u)path(t,x,u,path_data), input_path);
...
%   circle    
     a=10;
%     omega=0.01;
     pd=[a*cos(gamma/a);a*sin(gamma/a)];
%%   sin path    
...
phi=path_data(3);
d=path_data(4);
pd0=[a*cos(gamma0/a);a*sin(gamma0/a)]; 
psid0=atan2(cos(gamma0/a),-sin(gamma0/a));
...
function [solver,w0,lbw,ubw,lbg,ubg,nx,nu,k1,k2,k3]=NLP_gamma(Np,Tp,Ts,path_data)
...
% Input constraints
rmax=0.3;rmin=-0.3;
dgmax=ds_max;dgmin=ds_min;
umax=[rmax;dgmax];
umin=[rmin;dgmin];
% PF error dynamics equation
xdot = [vd*cos(psie)-u_g*(1-cg*y1);...
        vd*sin(psie)-cg*s1*u_g; ...
        r-cg*u_g;...
        u_g];

%% => gains for nonlinear controller >> scatter(getcolumn(x_robot(:,1:2),1),getcolumn(x_robot(:,1:2),2))

k1=(ds_max-vd_max);
k2=0.9*(rmax-cg_max*ds_max);
k3=0.1*(rmax-cg_max*ds_max)/(0.5*vd_max);

%% Objective term
Q=diag([1 1 1 1 10]);
L = [s1 y1 psie vd*cos(psie)-u_g r-cg*u_g]*Q*[s1;y1;psie;vd*cos(psie)-u_g;r-cg*u_g];
