...
%% Update path
input_path.u = u_g;
input_path.x = x_path(1:2);
output_path = RK4_integrator(@path, input_path);
gamma = output_path.value(1);
psi_d = output_path.value(2);
a=20;
omega=0.05;
pd=[a*sin(omega*gamma);gamma];
x_path=[gamma;psi_d;pd];
...
xdot = [vd*cos(psie)-h_g*u_g*(1-cg*y1);...
        vd*sin(psie)-cg*s1*h_g*u_g; ...
        r-cg*h_g*u_g;...
        u_g];
...
%% Objective term
Q=diag([1 1 1 1 5]);
L = [s1 y1 psie vd*cos(psie)-h_g*u_g r-cg*h_g*u_g]*Q*[s1;y1;psie;vd*cos(psie)-h_g*u_g;r-cg*h_g*u_g];
