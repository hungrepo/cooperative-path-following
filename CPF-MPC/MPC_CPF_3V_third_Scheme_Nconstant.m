...
Np = 5; % number of control intervals
Tp = Ts*Np; % Time horizon
import casadi.*
[nlp_sol1,w01,lbw1,ubw1,lbg1,ubg1,nx1,nu1,k1,k2,k3]=NLP(Np,Tp,Ts,path1_data);
[nlp_sol2,w02,lbw2,ubw2,lbg2,ubg2,nx2,nu2,k1,k2,k3]=NLP(Np,Tp,Ts,path2_data);
[nlp_sol3,w03,lbw3,ubw3,lbg3,ubg3,nx3,nu3,k1,k2,k3]=NLP(Np,Tp,Ts,path3_data);
...
gamma2=e_pf2(4);
gamma3=e_pf3(4);
%% Compute correction speed in coordination
L=[ 1  -1  0;        % network
   -1   2 -1;
    0  -1  1];
k=0.4;
...
e_gamma=L*gamma;
V_gamma=[V_gamma 0.5*(e_gamma')*e_gamma];
eta=-k*tanh(0.2*L*gamma);
uc1=eta(1); uc2=eta(2);uc3=eta(3);
uc=[uc eta];
x0_1=[e_pf1;uc1];
x0_2=[e_pf2;uc2];
x0_3=[e_pf3;uc3];
...
%% Compute controller for PF with MPC
...
%% Compute controller for PF with Lyapunov Based Controller
%     U1=0.5+uc1;
%     u_pf1=LPF(e_pf1,gain,path1_data,U1);
%     r_d1=u_pf1(1);
%     u_gamma1=u_pf1(2);
%     u_path1=[u_path1 u_gamma1];
% % Find PF control law for vehicle 2
%     U2=0.5+uc2;
%     u_pf2=LPF(e_pf2,gain,path2_data,U2);
%     r_d2=u_pf2(1);
%     u_gamma2=u_pf2(2);
%     u_path2=[u_path2 u_gamma2];
% % Find PF control law for vehicle 3
%     U3=0.5+uc3;
%     u_pf3=LPF(e_pf3,gain,path3_data,U3);
%     r_d3=u_pf3(1);
%     u_gamma3=u_pf3(2);
%     u_path3=[u_path3 u_gamma3];
%% Update the path and vehicle
...
xdot = [(vd+uc)*cos(psie)-hg_i*u_g*(1-cg_i*y1);...
        (vd+uc)*sin(psie)-cg_i*s1*hg_i*u_g; ...
        r-cg_i*hg_i*u_g;...
        u_g;...
        0];
...
k1=(ds_max-vd_max);
k2=0.4*(rmax-dgmax*cg_max*hg_max);
k3=0.6*(rmax-dgmax*cg_max*hg_max)/(0.5*vd_max);

%% Objective term
Q=diag([1 1 1 5 10]);
L = [s1, y1, psie, (vd+uc)*cos(psie)-hg_i*u_g, r-cg_i*hg_i*u_g]*Q*[s1;y1;psie;...
                                                                (vd+uc)*cos(psie)-hg_i*u_g;...
                                                                r-cg_i*hg_i*u_g];
...
function uL=LPF(epf,gain,path_data,U)
a=path_data(1);
omega=path_data(2);
phi=path_data(3);
k1=gain(1);
...
gamma_dot=U*cos(epf(3))+k1*tanh(epf(1));
gamma_dot=gamma_dot/hg;
den=1+epf(1)^2+epf(2)^2; 
if epf(3)==0
    tem=1;
else
    tem=sin(epf(3))/epf(3);
end
r=-k3*epf(2)*U*tem/den-k2*tanh(epf(3))+cg*hg*gamma_dot;
uL=[r;gamma_dot];
end