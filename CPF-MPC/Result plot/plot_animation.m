...
load Data_StrategyI_Constraint.mat
pd1=x_path1(:,1:2);
pd2=x_path2(:,1:2);
pd3=x_path3(:,1:2);
p1=x_robot1(:,1:2);
...
rd1=u_mpc(1,:);
rmax=0.1;
r_max=rmax*ones(1,nend);
r_min=-rmax*ones(1,nend);
t=t(1:nend);
e1=pd1-p1;
e2=pd2-p2;
k=length(e1(:,1));
...
%     subplot(2,1,2);
    ...
    hold on;
    plot(t,V2,'r');
    title('Position error between the path and the vehicle');
    ylabel('$$||e_{x},e_{y}||[m]$$','Interpreter','latex');
