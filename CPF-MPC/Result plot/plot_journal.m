...
% fig.PaperPosition = [0 0 6 3];      
    ...
    plot(pd1(193:end,2),pd1(193:end,1),'-','LineWidth',0.2,'Color',AUV_COL1);
    hold on;
    plot(pd2(58:end,2),pd2(58:end,1),'-','LineWidth',0.2,'Color',AUV_COL2);
...
%% Plot inputs
 ...
 set(fig3,'position',[0 0 560 300]);
  ...
  title('\bf Speed of the vehicles','Interpreter','latex');
...
%   set(gca,'YTick',[round(ey_min),0,round(ey_max)]);
  ...
  title('\bf Heading rate of the vehicles','Interpreter','latex');
  xlabel('t[second]','Interpreter','latex');
...
%   set(get(gca,'ylabel'),'rotation',0)
 ...
 set(fig4,'position',[0 0 560 300])
  ...
  title('\bf Evolution of the path parameters','Interpreter','latex');
  xlabel('t[second]','Interpreter','latex');
...
%   set(get(gca,'ylabel'),'rotation',0)
  ...
  subplot(4,1,1);
  idx=find(TR_1==1);
  plot(t(idx),1*TR_1(idx),'*','Color',AUV_COL1); hold on;
  idx=find(TR_2==1);
  plot(t(idx),0.7*TR_2(idx),'*','Color',AUV_COL2);
  idx=find(TR_3==1);
  plot(t(idx),0.4*TR_3(idx),'*','Color',AUV_COL3);
  tmin=t(1)-1; tmax=t(end)+1;
