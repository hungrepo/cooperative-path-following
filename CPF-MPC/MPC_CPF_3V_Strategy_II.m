...
import casadi.*
[nlp_sol1,w01,lbw1,ubw1,lbg1,ubg1,nx1,nu1,k31]=NLP1(Np,Tp,Ts,path1_data);
[nlp_sol2,w02,lbw2,ubw2,lbg2,ubg2,nx2,nu2,k32]=NLP2(Np,Tp,Ts,path2_data);
[nlp_sol3,w03,lbw3,ubw3,lbg3,ubg3,nx3,nu3,k33]=NLP1(Np,Tp,Ts,path3_data);
...
%% Formulate NLP
function [solver,w0,lbw,ubw,lbg,ubg,nx,nu,k3]=NLP1(Np,Tp,Ts,path_data)
...
hg_i=sqrt(1+a^2*omega^2*cos(omega*gamma_i+phi)^2);
hg_max=sqrt(1+a^2*omega^2);
cg_i=a*omega^2*sin(omega*gamma_i+phi)/(hg_i^3);
cg_max=a*omega^2;
...
ur_max=1; ur_min=0;
umax=[rmax;dgmax;ur_max];
umin=[rmin;dgmin;ur_min];
% PF error dynamics equation
xdot = [ur*cos(psie)-hg_i*u_g*(1-cg_i*y1);...
        ur*sin(psie)-cg_i*s1*hg_i*u_g;...
        r-cg_i*hg_i*u_g;...
        u_g;...
        vd];
...
R=1;
k=0.1;
L = [s1, y1, psie, ur*cos(psie)-hg_i*u_g, r-cg_i*hg_i*u_g]*Q*[s1;y1;psie;...
                                                                ur*cos(psie)-hg_i*u_g;...
                                                                r-cg_i*hg_i*u_g] + ...
    R*(ur-hg_i*(vd-k*tanh(gamma_i -gamma_j)))^2;
...
function [solver,w0,lbw,ubw,lbg,ubg,nx,nu,k3]=NLP2(Np,Tp,Ts,path_data)
...
psie = SX.sym('psie');
gamma_i = SX.sym('gamma_i');
gamma_j1=SX.sym('gamma_j1');
gamma_j2=SX.sym('gamma_j2');
% uc=SX.sym('uc');
x = [s1;y1;psie;gamma_i;gamma_j1;gamma_j2];
nx=length(x);
% input of PF error system 
r = SX.sym('r');
u_g=SX.sym('u_g');
...
L = [s1, y1, psie, ur*cos(psie)-hg_i*u_g, r-cg_i*hg_i*u_g]*Q*[s1;y1;psie;...
                                                                ur*cos(psie)-hg_i*u_g;...
                                                                r-cg_i*hg_i*u_g] + ...
R*(ur-hg_i*(vd-k*tanh(2*gamma_i -gamma_j1-gamma_j2)))^2;
%% Continuous time dynamics
f = Function('f', {x, u}, {xdot, L});
